...
- **AI设置**：配置提供商、API Key、模型等
- **主题设置**：支持亮色/暗色/跟随系统主题
- **语言设置**：控制AI输出语言及行为
- **存储设置**：管理数据存储位置

**响应式更新**：设置变更时实时应用到应用界面
...
- **设置存储**：统一保存在settings.json文件中
- **书籍索引**：保存在books_index.json文件中
- **书籍数据**：每本书独立目录，包含元数据和摘要
...
**API密钥保护**：AI配置信息保存在应用内部，不会泄露到版本控制
...
**多设置类别管理复杂性**：可能导致状态管理混乱
**应对措施**：统一设置服务，使用ValueNotifier实现响应式更新
